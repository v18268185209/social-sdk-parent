...
public final class SliderAntiDetect {
    ...
    public static String buildScript(long seed) {
        ...
        return ""
                ...
                + "          platform: uaData.platform || 'macOS',\n"
                + "          getHighEntropyValues: async (hints) => {\n"
                + "            const values = typeof uaData.getHighEntropyValues === 'function' ? await uaData.getHighEntropyValues(hints) : {};\n"
                + "            if (values && typeof values === 'object') {\n"
                + "              values.brands = (values.brands || []).map(b => ({ brand: String(b.brand || '').replace(/Headless/gi, ''), version: String(b.version || '') }));\n"
                + "              values.platform = values.platform || 'macOS';\n"
                + "            }\n"
                + "            return values;\n"
                + "          }\n"
                + "        })\n"
    ...
    // LEGACY_SCRIPT_START 已移除：旧版字面量脚本统一由 buildScript(DEFAULT_SEED) 生成。
