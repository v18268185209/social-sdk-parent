...
/**
 * MySQL 实现的 {@link BindingRepository}。与应用共用同一个 MySQL 库，表名前缀 {@code proxy_}。
 *
 * <p>通过构造传入 {@link DataSource}，与应用库复用同一个连接池（Druid/HikariCP）。</p>
 *
 * <p>所有方法都 synchronous，调用方应自行切换线程池。</p>
 */
public class MySqlBindingRepository implements BindingRepository {

    private static final Logger log = LoggerFactory.getLogger(MySqlBindingRepository.class);
    ...
    private static final String INSERT_SQL = "INSERT INTO " + TABLE
            ...
            + " provider_type=VALUES(provider_type), host=VALUES(host), port=VALUES(port), "
            + " username=VALUES(username), password=VALUES(password), exit_ip=VALUES(exit_ip), "
            + " city=VALUES(city), last_used_at=VALUES(last_used_at), use_count=VALUES(use_count), "
            + " captcha_passed=VALUES(captcha_passed), deleted=VALUES(deleted)";
    ...
    public MySqlBindingRepository(DataSource dataSource) {
    ...
    private void createIndexIfMissing(Connection conn, String indexName, String createSql) throws SQLException {
        // MySQL 用 information_schema.statistics 查索引
