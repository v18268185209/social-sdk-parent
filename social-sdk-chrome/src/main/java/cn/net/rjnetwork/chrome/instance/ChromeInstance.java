package cn.net.rjnetwork.chrome.instance;
...
import cn.net.rjnetwork.chrome.config.FingerprintConfig;
import cn.net.rjnetwork.chrome.fingerprint.FingerprintSpoofer;
...
public class ChromeInstance implements AutoCloseable {
    ...
    /**
     * 获取原生 CDP 客户端，复用当前 Chrome 实例的 DevTools WebSocket。
     *
     * <p>这是 SDK 缺失已久的「原始 CDP 通路」：在保留 Selenium 驱动的同时，
     * 可通过 {@link CdpClient} 直接调用 Fetch 拦截、UA/请求头覆盖、Emulation
     * （地理/时区/触摸）、Performance、DOMSnapshot、Storage 清理等能力。
     * 若实例未就绪或不是 ChromeDriver，返回空。</p>
     */
    public Optional<CdpClient> getCdpClient() {
        if (!(driver instanceof ChromeDriver)) {
            return Optional.empty();
        }
        try {
            Object cdpUrl = ((ChromeDriver) driver).getCapabilities().getCapability("se:cdp");
            if (cdpUrl == null) {
                return Optional.empty();
            }
            return Optional.of(CdpClient.connect(cdpUrl.toString()));
        } catch (Exception e) {
            logger.warn("Failed to create CdpClient for instance {}: {}", instanceName, e.getMessage());
            return Optional.empty();
        }
    }
